...
Vector findPath(City c1, City c2) {
	...
	if (c2->numberOfPaths == 2)
		addElement(result, NULL);
...
Vector getBetterPath(Vector path1, Vector path2) {
	...
	if (path1->numberOfElements == 0 && path2->numberOfElements == 0)
		return NULL;
	if (path1->numberOfElements == 0 && path2->numberOfElements % 2 == 1)
		return copyVector(path2);
	if (path1->numberOfElements % 2 == 1 && path2->numberOfElements == 0)
	...
	if (lengthOfPath1 < lengthOfPath2 ||
		...
		if (path1->numberOfElements % 2 == 1)
			return copyVector(path1);
		else
			return NULL;
	}

	if (path2->numberOfElements % 2 == 1)
		return copyVector(path2);
	else
		return NULL;
}
